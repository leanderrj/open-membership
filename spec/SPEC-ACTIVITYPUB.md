...
ActivityPub specifies a federated social protocol but is silent on paid content. Existing deployments handle monetisation per-server, with bespoke integrations (Patreon-bridged Mastodon accounts, Stripe links in profile fields, server-level ACLs wrapped around PayPal subscriptions). Ghost ships native ActivityPub; WordPress carries the Automattic ActivityPub plugin. When a publisher running either stack federates an actor while also gating a post with `<om:access>locked</om:access>`, two failure modes follow absent a shared convention:

1. The full gated content federates to every follower's home server, breaking the paywall.
2. Federation is disabled for gated posts, eliminating the federated-discovery story.

This appendix specifies the convention that avoids both. It defines how an Open Membership entitlement is carried alongside an ActivityPub authenticated-fetch request, and how a publisher's outbox represents a gated post without leaking content. The shape is parallel to the Podcasting 2.0 coexistence rules in [`SPEC.md`](../SPEC.md) §8: a neighbouring namespace continues to function in the presence of `om:` markup without either side needing to redesign.
...
The implication is that `pseudonymous-required` publishers and ActivityPub are incompatible at the protocol layer for the gated half of their catalogue. This appendix does not attempt to resolve the incompatibility: two valid designs exist (federate-and-paywall, or privacy-first-no-federation), and the publisher picks one per post or per catalogue.
